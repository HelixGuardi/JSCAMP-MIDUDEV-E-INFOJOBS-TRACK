...
exports.captureHybridSnapshot = captureHybridSnapshot;
exports.buildFrameContext = buildFrameContext;
exports.tryScopedSnapshot = tryScopedSnapshot;
exports.buildSessionIndexes = buildSessionIndexes;
exports.collectPerFrameMaps = collectPerFrameMaps;
exports.resolveIgnoredNodes = resolveIgnoredNodes;
exports.buildFrameExclusionIntervals = buildFrameExclusionIntervals;
exports.computeFramePrefixes = computeFramePrefixes;
exports.mergeFramesIntoSnapshot = mergeFramesIntoSnapshot;
const frame_js_1 = require("../../frame.js");
const selectorResolver_js_1 = require("../../selectorResolver.js");
const logger_js_1 = require("../../../logger.js");
const a11yTree_js_1 = require("./a11yTree.js");
const focusSelectors_js_1 = require("./focusSelectors.js");
const domTree_js_1 = require("./domTree.js");
const treeFormatUtils_js_1 = require("./treeFormatUtils.js");
 ...
 */
async function captureHybridSnapshot(page, options) {
 ...
 */
async function tryScopedSnapshot(page, options, context, pierce, sessionToIndex, exclusionIntervalsByFrame) {
    ...
    try {
        ...
        if (looksLikeXPath) {
            const focus = (0, xpathUtils_js_1.normalizeXPath)(requestedFocus);
            const hit = await (0, focusSelectors_js_1.resolveFocusFrameAndTail)(page, focus, context.parentByFrame, context.rootId);
            targetFrameId = hit.targetFrameId;
            tailSelector = hit.tailXPath || undefined;
            absPrefix = hit.absPrefix;
        }
        else {
            const cssHit = await (0, focusSelectors_js_1.resolveCssFocusFrameAndTail)(page, requestedFocus, context.parentByFrame, context.rootId);
            targetFrameId = cssHit.targetFrameId;
            tailSelector = cssHit.tailSelector || undefined;
            absPrefix = cssHit.absPrefix;
        }
        const owningSess = (0, sessions_js_1.ownerSession)(page, targetFrameId);
        const parentId = context.parentByFrame.get(targetFrameId);
        const sameSessionAsParent = !!parentId &&
            (0, sessions_js_1.ownerSession)(page, parentId) === (0, sessions_js_1.ownerSession)(page, targetFrameId);
        const { tagNameMap, xpathMap, scrollableMap } = await (0, domTree_js_1.domMapsForSession)(owningSess, targetFrameId, pierce, (fid, be) => `${page.getOrdinal(fid)}-${be}`, sameSessionAsParent);
        const { outline, urlMap, scopeApplied } = await (0, a11yTree_js_1.a11yForFrame)(owningSess, targetFrameId, {
        ...
        if (isRoot) {
            ...
            for (const [encId, xp] of Object.entries(xpathMap)) {
                ...
                scopedXpathMap[encId] = (0, xpathUtils_js_1.prefixXPath)(abs, xp);
            }
        }
        const scopedUrlMap = { ...urlMap };
 ...
 */
async function buildSessionIndexes(page, frames, pierce) {
    const sessionToIndex = new Map();
    const sessionById = new Map();
    for (const frameId of frames) {
        const sess = (0, sessions_js_1.ownerSession)(page, frameId);
        const sid = sess.id ?? "root";
        if (!sessionById.has(sid))
            sessionById.set(sid, sess);
    }
    for (const [sid, sess] of sessionById.entries()) {
        const idx = await (0, domTree_js_1.buildSessionDomIndex)(sess, pierce);
 ...
 */
async function collectPerFrameMaps(page, context, sessionToIndex, options, pierce, frameIds, exclusionIntervalsByFrame) {
    ...
    for (const frameId of frameIds) {
        ...
        if (!idx) {
            idx = await (0, domTree_js_1.buildSessionDomIndex)(sess, pierce);
            sessionToIndex.set(sid, idx);
        }
        const parentId = context.parentByFrame.get(frameId);
        const sameSessionAsParent = !!parentId && (0, sessions_js_1.ownerSession)(page, parentId) === sess;
        ...
        for (const [be, nodeAbs] of idx.absByBe.entries()) {
            ...
            const rel = (0, domTree_js_1.relativizeXPath)(baseAbs, nodeAbs);
            const key = enc(be);
            xpathMap[key] = rel;
        ...
        const { outline, urlMap } = await (0, a11yTree_js_1.a11yForFrame)(sess, frameId, {
...
async function resolveIgnoredNodes(page, ignoreSelectors, context, sessionToIndex) {
...
async function resolveIgnoredNodesForSelector(page, selector, context, sessionToIndex) {
    ...
    if (looksLikeXPath) {
        const hit = await (0, focusSelectors_js_1.resolveFocusFrameAndTail)(page, (0, xpathUtils_js_1.normalizeXPath)(selector), context.parentByFrame, context.rootId);
    ...
    const hit = await (0, focusSelectors_js_1.resolveCssFocusFrameAndTail)(page, selector, context.parentByFrame, context.rootId);
    const targetFrameId = hit.targetFrameId;
    return resolveIgnoredNodesInFrame(page, targetFrameId, {
        kind: "css",
...
async function resolveIgnoredNodesInFrame(page, frameId, query) {
    const session = (0, sessions_js_1.ownerSession)(page, frameId);
    const frame = new frame_js_1.Frame(session, frameId, "", false);
    const resolver = new selectorResolver_js_1.FrameSelectorResolver(frame);
...
async function buildFrameExclusionIntervals(page, context, sessionToIndex, ignoredNodesByFrame) {
    ...
    const excludeFrameSubtree = async (frameId) => {
        ...
        const parentId = context.parentByFrame.get(frameId);
        const sameSessionAsParent = !!parentId &&
            (0, sessions_js_1.ownerSession)(page, parentId) === (0, sessions_js_1.ownerSession)(page, frameId);
        const docRootBe = await resolveFrameDocRootBackendId(page, frameId, idx, sameSessionAsParent);
        ...
        for (const childFrameId of (0, focusSelectors_js_1.listChildrenOf)(context.parentByFrame, frameId)) {
...
async function resolveChildFramesByParent(page, context) {
    ...
    for (const frameId of context.frames) {
        ...
        if (!parentId)
            continue;
        const session = (0, sessions_js_1.parentSession)(page, context.parentByFrame, frameId);
        if (!session)
            continue;
        try {
 ...
 */
async function computeFramePrefixes(page, context, perFrameMaps, frameIds) {
    ...
    while (queue.length) {
        ...
        for (const child of context.frames) {
            ...
            if (context.parentByFrame.get(child) !== parent)
                continue;
            queue.push(child);
            const parentSess = (0, sessions_js_1.parentSession)(page, context.parentByFrame, child);
            const ownerBackendNodeId = await (async () => {
                try {
                    const { backendNodeId } = await parentSess.send("DOM.getFrameOwner", { frameId: child });
                    return backendNodeId;
                }
                catch {
                    return undefined;
            ...
            const childAbs = iframeXPath
                ? (0, xpathUtils_js_1.prefixXPath)(parentAbs || "/", iframeXPath)
                : parentAbs;
 ...
 */
function mergeFramesIntoSnapshot(context, perFrameMaps, perFrameOutlines, absPrefix, iframeHostEncByChild, frameIds) {
    ...
    for (const frameId of frameIds) {
        ...
        for (const [encId, xp] of Object.entries(maps.xpathMap)) {
            combinedXpathMap[encId] = (0, xpathUtils_js_1.prefixXPath)(abs, xp);
        }
        Object.assign(combinedUrlMap, maps.urlMap);
    }
    ...
    const rootOutline = perFrameOutlines.find((o) => o.frameId === context.rootId)?.outline ??
        perFrameOutlines[0]?.outline ??
        "";
    const combinedTree = (0, treeFormatUtils_js_1.injectSubtrees)(rootOutline, idToTree).toWellFormed();
    return {
        combinedTree,
        combinedXpathMap,
        combinedUrlMap,
