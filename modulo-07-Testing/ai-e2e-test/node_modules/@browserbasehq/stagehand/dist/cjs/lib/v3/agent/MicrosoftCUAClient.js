...
exports.MicrosoftCUAClient = void 0;
 ...
 */
class MicrosoftCUAClient extends AgentClient_js_1.AgentClient {
    apiKey;
    baseURL;
    client;
...
{{"name": <function-name>, "arguments": <args-json-object>}}
    ...
    parseThoughtsAndAction(response) {
        try {
            ...
            try {
                ...
                if (actionText.startsWith("{{") && actionText.endsWith("}")) {
                    ...
                    try {
                        ...
                        throw new Error(`Failed to parse action text even after fixing double brackets. Original: ${actionText}. Fixed: ${fixedText}. Error: ${retryError}`, { cause: retryError });
                    }
                }
                else {
                    throw new Error(`Failed to parse action text as JSON: ${actionText}. Error: ${jsonError}`, { cause: jsonError });
                }
            }
            return {
                thoughts,
            ...
            throw new Error(`Failed to parse FARA tool call from response: ${response}. Error: ${error}`, { cause: error });
        }
    }
    /**
    ...
    convertFunctionCallToAction(functionCall) {
        ...
        switch (action) {
            ...
            case "keypress": {
                ...
                const normalizedKeys = keys.map((k) => (0, cuaKeyMapping_js_1.mapKeyToPlaywright)(k));
                return {
                    ...baseAction,
                    type: "keypress",
                    keys: normalizedKeys,
...
exports.MicrosoftCUAClient = MicrosoftCUAClient;
