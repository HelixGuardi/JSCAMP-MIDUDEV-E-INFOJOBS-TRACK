...
const postStepProbeEvidence_js_1 = require("../agent/utils/postStepProbeEvidence.js");
const wrapEvidenceCallback_js_1 = require("../agent/utils/wrapEvidenceCallback.js");
const toolOutputEvidence_js_1 = require("../agent/utils/toolOutputEvidence.js");
...
class V3CuaAgentHandler {
    ...
    setupAgentClient() {
        ...
        this.agentClient.setActionHandler(async (action) => {
            ...
            try {
                ...
                if (shouldLog) {
                    executionResult = await FlowLogger_js_1.FlowLogger.runWithLogging({
                ...
                if (shouldLog && this.evidenceCallback) {
                    await this.emitCuaActionStep(action, (0, toolOutputEvidence_js_1.inferToolOutput)(executionResult ?? { success: true }));
                }
                ...
                if (shouldLog && this.evidenceCallback) {
                    try {
                        ...
                        this.logger({
                            category: "agent",
                            message: `Failed to record failed-action evidence: ${evidenceError instanceof Error
    ...
    async execute(optionsOrInstruction) {
        ...
        this.evidenceCallback = (0, wrapEvidenceCallback_js_1.wrapEvidenceCallback)(options.callbacks?.onEvidence, this.logger);
        ...
        try {
            ...
            if (this.evidenceCallback) {
                ...
                const observation = await (0, postStepProbeEvidence_js_1.captureProbeEvidence)({
                    v3: this.v3,
                    url: finalUrl,
                    logger: this.logger,
            ...
            case "keypress": {
                ...
                if (keyList.length > 0) {
                    ...
                    const mapped = keyList
                        .map((rawKey) => (0, cuaKeyMapping_js_1.mapKeyToPlaywright)(String(rawKey ?? "")))
                        .join("+");
     ...
     */
    async emitCuaScreenshot(screenshot, url) {
        if (!this.evidenceCallback)
            return;
    ...
    async emitCuaActionStep(action, toolOutput) {
        let pageUrl = typeof action.pageUrl === "string"
            ? action.pageUrl
        ...
        const actionArgs = Object.fromEntries(Object.entries(action).filter(([key]) => key !== "screenshot" && key !== "timestamp"));
        const reasoning = typeof action.reasoning === "string"
            ? action.reasoning
        ...
        await (0, postStepProbeEvidence_js_1.emitPostStepProbeEvidence)({
            v3: this.v3,
            url: pageUrl,
