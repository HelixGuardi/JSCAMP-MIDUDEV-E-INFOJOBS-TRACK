...
exports.normalizeDomainPolicy = normalizeDomainPolicy;
exports.getDomainPolicyDecision = getDomainPolicyDecision;
...
function validateHostname(hostname, original, kind) {
    const label = kind === "allowedDomains" ? "allowed" : "blocked";
    if (!hostname || hostname.length > 253) {
        throw new sdkErrors_js_1.StagehandInvalidArgumentError(`Invalid ${label} domain pattern: "${original}"`);
    }
...
function normalizeDomainPattern(pattern, kind) {
    const label = kind === "allowedDomains" ? "allowed" : "blocked";
    const capitalizedLabel = kind === "allowedDomains" ? "Allowed" : "Blocked";
    if (typeof pattern !== "string") {
        throw new sdkErrors_js_1.StagehandInvalidArgumentError(`${capitalizedLabel} domain patterns must be strings`);
    }
    ...
    if (normalized.includes("://") ||
        ...
        normalized.includes("?") ||
        normalized.includes("#")) {
        throw new sdkErrors_js_1.StagehandInvalidArgumentError(`${capitalizedLabel} domain patterns must be domain-only values: "${original}"`);
    }
    ...
    if (normalized.includes("*")) {
        throw new sdkErrors_js_1.StagehandInvalidArgumentError(`Wildcards are only supported as a leading "*.": "${original}"`);
    }
    validateHostname(normalized, original, kind);
    return { type: "exact", hostname: normalized };
}
function normalizeDomainPatterns(patterns, kind) {
    if (!patterns?.length)
        return [];
...
function getDomainPatterns(policy, key) {
    if (policy === null)
        return [];
    ...
    if (policy === undefined) {
        throw new sdkErrors_js_1.StagehandInvalidArgumentError("Domain policy must be an object or null");
    }
    ...
    if (!Array.isArray(patterns)) {
        throw new sdkErrors_js_1.StagehandInvalidArgumentError(`${key} must be an array of strings`);
    }
    return patterns;
}
function patternHost(rule) {
    return rule.type === "wildcard" ? `*.${rule.hostname}` : rule.hostname;
}
function fetchPatternHosts(rule) {
    const host = patternHost(rule);
    return [host, `${host}.`];
}
function toBlocklistFetchPatterns(rules) {
    const patterns = [];
...
function toAllowlistFetchPatterns() {
    return HTTP_SCHEMES.map((scheme) => ({
        urlPattern: `${scheme}://*/*`,
        requestStage: "Request",
    }));
}
function normalizeDomainPolicy(policy) {
...
function hostnameFromHttpUrl(url) {
    try {
...
function matchesDomainRules(hostname, rules) {
...
function getDomainPolicyDecision(url, policy) {
    if (!policy)
    ...
    if (policy.allowedDomainRules.length &&
