...
exports.resolveXPathFirst = resolveXPathFirst;
exports.resolveXPathAtIndex = resolveXPathAtIndex;
exports.countXPathMatches = countXPathMatches;
exports.resolveXPathComposedMatches = resolveXPathComposedMatches;
const xpathParser_js_1 = require("./xpathParser.js");
...
function resolveXPathComposedMatches(rawXp, getClosedRoot) {
    const xp = normalizeXPath(rawXp);
    if (!xp)
        return [];
    const steps = (0, xpathParser_js_1.parseXPathSteps)(xp);
    if (!steps.length)
        return [];
    ...
    for (const step of steps) {
        ...
        for (const root of current) {
            ...
            const matches = (0, xpathParser_js_1.applyPredicates)(tagMatches, step.predicates);
...
function getShadowContext() {
    ...
    const getClosedRoot = backdoor && typeof backdoor.getClosedRoot === "function"
        ? (host) => {
            try {
...
function resolveNativeAtIndexWithError(xp, index) {
    try {
        ...
        return {
            value: snapshot.snapshotItem(index),
            error: false,
        };
    }
    catch {
...
//# sourceMappingURL=xpathResolver.js.map