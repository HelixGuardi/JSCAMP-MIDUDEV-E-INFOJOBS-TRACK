...
exports.CdpSession = exports.CdpConnection = void 0;
...
class CdpConnection {
    ...
    clearSessionEventHandlers(sessionId) {
    ...
    rejectSessionPendingWork(sessionId, targetId) {
        for (const [id, entry] of this.inflight.entries()) {
            if (entry.sessionId === sessionId) {
                entry.reject(new sdkErrors_js_1.PageNotFoundError(`target closed before CDP response (sessionId=${sessionId}, targetId=${targetId})`));
                this.inflight.delete(id);
            }
        }
        for (const waiter of Array.from(this.sessionDispatchWaiters)) {
            if (waiter.sessionId === sessionId) {
                waiter.reject(new sdkErrors_js_1.PageNotFoundError(`target closed before CDP send (sessionId=${sessionId}, targetId=${targetId})`));
            }
        }
    }
