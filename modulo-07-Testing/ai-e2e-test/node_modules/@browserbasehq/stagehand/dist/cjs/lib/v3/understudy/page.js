...
const index_js_1 = require("./a11y/snapshot/index.js");
const frameRegistry_js_1 = require("./frameRegistry.js");
...
const navigationResponseTracker_js_1 = require("./navigationResponseTracker.js");
const response_js_1 = require("./response.js");
const consoleMessage_js_1 = require("./consoleMessage.js");
const index_js_2 = require("../types/public/index.js");
const sdkErrors_js_1 = require("../types/public/sdkErrors.js");
const initScripts_js_1 = require("./initScripts.js");
const locatorInvocation_js_1 = require("./locatorInvocation.js");
const screenshotUtils_js_1 = require("./screenshotUtils.js");
...
const WEB_MCP_SUPPORT_MESSAGE = "Make sure you are using Chrome/Chromium newer than version 149. Stagehand's local Chrome launcher enables WebMCP flags by default; if you are connecting to an existing browser or remote CDP endpoint, launch it with --enable-features=WebMCPTesting,DevToolsWebMCPSupport.";
...
let Page = (() => {
    ...
    return class Page {
        ...
        constructor(conn, mainSession, _targetId, mainFrameId, apiClient, browserIsRemote = false) {
            ...
            this.registry = new frameRegistry_js_1.FrameRegistry(_targetId, mainFrameId);
            // main-frame wrapper is always bound to the **top-level** session
            this.mainFrameWrapper = new frame_js_1.Frame(this.mainSession, mainFrameId, this.pageId, this.browserIsRemote);
            this.networkManager = new networkManager_js_1.NetworkManager();
      ...
      if (document.readyState === 'complete' || document.readyState === 'interactive') {
        ...
        async addInitScript(script, arg) {
            const source = await (0, initScripts_js_1.normalizeInitScriptSource)(script, arg, "page.addInitScript");
        ...
        onFrameNavigated(frame, session) {
            ...
            if (newRoot !== prevRoot) {
                ...
                this.mainFrameWrapper = new frame_js_1.Frame(this.mainSession, newRoot, this.pageId, this.browserIsRemote);
            }
        ...
        frameForId(frameId) {
            ...
            const f = new frame_js_1.Frame(sess, frameId, this.pageId, this.browserIsRemote);
        ...
        on(event, listener) {
            if (event !== "console") {
                throw new sdkErrors_js_1.StagehandInvalidArgumentError(`Unsupported event: ${event}`);
            }
        ...
        emitConsole(evt) {
            if (this.consoleListeners.size === 0)
                return;
            const message = new consoleMessage_js_1.ConsoleMessage(evt, this);
        ...
        async goto(url, options) {
            ...
            const tracker = new navigationResponseTracker_js_1.NavigationResponseTracker({
                page: this,
                session: this.mainSession,
                navigationCommandId,
            });
            const watcher = new lifecycleWatcher_js_1.LifecycleWatcher({
                page: this,
            ...
            try {
                ...
                if (this.apiClient) {
                    ...
                    if ((0, response_js_1.isSerializableResponse)(result)) {
                        return response_js_1.Response.fromSerializable(result, {
                            page: this,
                            session: this.mainSession,
                        });
                    }
        ...
        async reload(options) {
            ...
            const watcher = waitUntil
                ? new lifecycleWatcher_js_1.LifecycleWatcher({
                    page: this,
                    mainSession: this.mainSession,
                    networkManager: this.networkManager,
                    waitUntil,
        ...
        async screenshot(options) {
            ...
            if (type !== "png" && type !== "jpeg") {
                throw new sdkErrors_js_1.StagehandInvalidArgumentError(`screenshot: unsupported image type "${type}"`);
            }
            if (opts.fullPage && opts.clip) {
                throw new sdkErrors_js_1.StagehandInvalidArgumentError("screenshot: clip and fullPage cannot be used together");
            }
            if (type === "png" && typeof opts.quality === "number") {
                throw new sdkErrors_js_1.StagehandInvalidArgumentError('screenshot: quality option is only valid for type="jpeg"');
            }
            const caretMode = opts.caret ?? "hide";
            const animationsMode = opts.animations ?? "allow";
            const scaleMode = opts.scale ?? "device";
            const frames = (0, screenshotUtils_js_1.collectFramesForScreenshot)(this);
            const clip = opts.clip ? (0, screenshotUtils_js_1.normalizeScreenshotClip)(opts.clip) : undefined;
            const captureScale = await (0, screenshotUtils_js_1.computeScreenshotScale)(this, scaleMode);
            const maskLocators = (opts.mask ?? []).filter((locator) => Boolean(locator));
            const cleanupTasks = [];
            const exec = async () => {
                try {
                    if (opts.omitBackground) {
                        cleanupTasks.push(await (0, screenshotUtils_js_1.setTransparentBackground)(this.mainSession));
                    }
                    if (animationsMode === "disabled") {
                        cleanupTasks.push(await (0, screenshotUtils_js_1.disableAnimations)(frames));
                    }
                    if (caretMode === "hide") {
                        cleanupTasks.push(await (0, screenshotUtils_js_1.hideCaret)(frames));
                    }
                    if (opts.style && opts.style.trim()) {
                        cleanupTasks.push(await (0, screenshotUtils_js_1.applyStyleToFrames)(frames, opts.style, "custom"));
                    }
                    if (maskLocators.length > 0) {
                        cleanupTasks.push(await (0, screenshotUtils_js_1.applyMaskOverlays)(maskLocators, opts.maskColor ?? "#FF00FF"));
                    }
                    ...
                    if (opts.path) {
                        await fs_1.promises.writeFile(opts.path, buffer);
                    }
                    return buffer;
                }
                finally {
                    await (0, screenshotUtils_js_1.runScreenshotCleanups)(cleanupTasks);
                }
            };
            return await (0, timeoutConfig_js_1.withTimeout)(exec(), opts.timeout, "screenshot");
        }
        /**
        ...
        async setExtraHTTPHeaders(headers) {
            ...
            if (errors.length > 0) {
                throw new index_js_2.StagehandSetExtraHTTPHeadersError(errors);
            }
        }
        /**
        ...
        deepLocator(selector) {
            return (0, deepLocator_js_1.deepLocatorFromPage)(this, this.mainFrameWrapper, selector);
        }
        /**
        ...
        frameLocator(selector) {
            return new frameLocator_js_1.FrameLocator(this, selector);
        }
        /**
        ...
        async waitForSelector(selector, options) {
            ...
            const { frame: targetFrame, selector: finalSelector } = await (0, deepLocator_js_1.resolveLocatorTarget)(this, root, selector);
            const elapsed = Date.now() - startTime;
            const remainingTimeout = Math.max(0, timeout - elapsed);
            const expression = (0, locatorInvocation_js_1.buildLocatorInvocation)("waitForSelector", [
          ...
          try {
            ...
            if (exceptionDetails) {
                const msg = exceptionDetails.text ||
                    ...
                    "Evaluation failed";
                throw new sdkErrors_js_1.StagehandEvalError(msg);
            }
            return result?.value;
        }
        /**
        ...
        async dragAndDrop(fromX, fromY, toX, toY, options) {
            ...
            if (options?.returnXpath) {
                try {
                    const start = await (0, index_js_1.resolveXpathForLocation)(this, fromX, fromY);
                    if (start)
                ...
                try {
                    const end = await (0, index_js_1.resolveXpathForLocation)(this, toX, toY);
                    if (end)
        ...
        async snapshot(options) {
            try {
                const { combinedTree, combinedXpathMap, combinedUrlMap } = await (0, index_js_1.captureHybridSnapshot)(this, {
                    pierceShadow: true,
                ...
                throw new index_js_2.StagehandSnapshotError(err);
            }
        }
        ...
        async mainWorldExecutionContextId() {
            return executionContextRegistry_js_1.executionContexts.waitForMainWorld(this.mainSession, this.mainFrameId(), 1000);
        }
