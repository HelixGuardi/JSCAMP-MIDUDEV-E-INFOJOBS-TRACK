...
const rubricVerifier_js_1 = require("./v3/verifier/rubricVerifier.js");
...
class V3Evaluator {
    ...
    async verify(trajectory) {
        ...
        const verifier = new rubricVerifier_js_1.RubricVerifier({
    ...
    unavailableVerifierBackend(methodName) {
        throw new sdkErrors_js_1.StagehandInvalidArgumentError(`V3Evaluator.${methodName}() was configured with ${EVALUATOR_BACKEND_ENV}=verifier, but the verifier backend only supports verify() and generateRubric(). Use "legacy" for ask()/batchAsk().`);
    }
    getClient() {
