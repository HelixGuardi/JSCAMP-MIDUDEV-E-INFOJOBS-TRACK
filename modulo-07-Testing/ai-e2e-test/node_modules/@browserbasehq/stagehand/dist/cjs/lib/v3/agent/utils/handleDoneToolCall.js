...
exports.sanitizeMessagesForResubmission = sanitizeMessagesForResubmission;
exports.handleDoneToolCall = handleDoneToolCall;
...
function sanitizeMessagesForResubmission(messages) {
    return stripUndefinedDeep(messages);
}
function buildBaseDoneSchema(factory) {
 ...
 */
async function handleDoneToolCall(options) {
    ...
    const factory = outputSchema
        ? (0, utils_js_1.getZFactory)(outputSchema)
        : zod_1.z;
    ...
    const doneToolSchema = outputSchema
        ? baseDoneSchema.extend({
            output: outputSchema.describe("The specific data the user requested from this task"),
        })
        : baseDoneSchema;
    const outputInstructions = outputSchema
        ? `\n\nThe user also requested the following information from this task. Provide it in the "output" field:\n${JSON.stringify(Object.fromEntries(Object.entries(outputSchema.shape).map(([key, value]) => [
            key,
...
2. Whether the task was completed successfully${outputSchema ? "\n3. The requested output data based on what you found" : ""}`;
    ...
    const result = await (0, ai_1.generateText)({
        ...
        tools: { done: doneTool },
        toolChoice: (0, anthropicOptions_js_1.rejectsForcedToolUse)(modelId)
            ? "auto"
    ...
    const input = doneToolCall.input;
    logger({
        category: "agent",
...
//# sourceMappingURL=handleDoneToolCall.js.map