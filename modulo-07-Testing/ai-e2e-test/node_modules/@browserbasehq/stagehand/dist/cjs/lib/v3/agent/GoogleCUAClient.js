 ...
 */
class GoogleCUAClient extends AgentClient_js_1.AgentClient {
    apiKey;
    client;
    ...
    constructor(type, modelName, userProvidedInstructions, clientOptions, tools) {
        ...
        this.client = new genai_1.GoogleGenAI(genAIOptions);
    ...
    updateGenerateContentConfig() {
        const functionDeclarations = this.tools && Object.keys(this.tools).length > 0
            ? (0, googleCustomToolHandler_js_1.convertToolSetToFunctionDeclarations)(this.tools)
            : [];
        this.generateContentConfig = {
            ...this.generateContentConfig,
            tools: [
    ...
    async initializeHistory(instruction) {
        ...
        const systemPromptContent = this.userProvidedInstructions
            ...
            : (0, prompt_js_1.buildGoogleCUASystemPrompt)().content;
        this.history = [
            {
                role: "user",
                parts: [
    ...
    async executeStep(logger) {
        try {
            ...
            const compressedResult = (0, imageCompression_js_1.compressGoogleConversationImages)(this.history, 2);
            ...
            FlowLogger_js_1.FlowLogger.logLlmRequest({
                ...
                prompt: (0, FlowLogger_js_1.extractLlmCuaPromptSummary)(compressedHistory),
            });
            for (let attempt = 0; attempt < maxRetries; attempt++) {
                try {
                    ...
                    if (!response.candidates || response.candidates.length === 0) {
                        throw new sdkErrors_js_1.LLMResponseError("agent", "Response has no candidates!");
                    }
                    const candidate = response.candidates[0];
                    if (!candidate.content || !candidate.content.parts) {
                        const reason = candidate.finishReason || "unknown";
                        throw new sdkErrors_js_1.LLMResponseError("agent", `Response has no content (finish reason: ${reason})`);
                    }
            ...
            FlowLogger_js_1.FlowLogger.logLlmResponse({
                requestId: llmRequestId,
                model: this.modelName,
                output: (0, FlowLogger_js_1.extractLlmCuaResponseSummary)(response),
            ...
            // model blind and it gives up after one empty turn.
            {
                    ...
                    if (action.type === "open_web_browser") {
                        ...
                        if (this.tools && toolName in this.tools) {
                            ...
                            if (correspondingFunctionCall) {
                                const executionResult = await (0, googleCustomToolHandler_js_1.executeGoogleCustomTool)(toolName, toolArgs, this.tools, correspondingFunctionCall, logger);
                        ...
                        try {
                            ...
                            if (actionError instanceof sdkErrors_js_1.StagehandClosedError) {
                                throw actionError;
                            }
                            logger({
                                category: "agent",
                ...
                if (result.functionCalls.length > 0 || hasError) {
                    ...
                    const computerUseFunctionCalls = result.functionCalls.filter((fc) => !(0, googleCustomToolHandler_js_1.isCustomTool)(fc, this.tools));
    ...
    convertFunctionCallToAction(functionCall, logger) {
        ...
        switch (name) {
            ...
            default:
                if ((0, googleCustomToolHandler_js_1.isCustomTool)(functionCall, this.tools)) {
                    return {
                        type: "custom_tool",
                        name,
                        arguments: args,
