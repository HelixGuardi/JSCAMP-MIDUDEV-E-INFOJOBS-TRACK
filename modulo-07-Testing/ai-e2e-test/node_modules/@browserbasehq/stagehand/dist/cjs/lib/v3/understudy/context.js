...
function getFirstTopLevelPageTimeoutMs() {
    return ((0, timeoutConfig_js_1.getEnvTimeoutMs)(FIRST_TOP_LEVEL_PAGE_TIMEOUT_ENV) ??
        (process.env.CI
...
class V3Context {
    ...
    static async create(wsUrl, opts) {
        const connectTask = async () => {
            ...
            const firstPageTimeoutMs = Math.max(opts?.localBrowserLaunchOptions?.connectTimeoutMs ?? 0, getFirstTopLevelPageTimeoutMs());
            await ctx.ensureFirstTopLevelPage(firstPageTimeoutMs);
            return ctx;
        };
        const cdpTimeoutMs = opts?.env === "BROWSERBASE"
            ? (0, timeoutConfig_js_1.getEnvTimeoutMs)("BROWSERBASE_CDP_CONNECT_MAX_MS")
            : undefined;
        if (cdpTimeoutMs) {
            let timedOut = false;
            const connectPromise = connectTask();
            const guarded = (0, timeoutConfig_js_1.withTimeout)(connectPromise, cdpTimeoutMs, "Browserbase CDP connect").catch((err) => {
                timedOut = true;
                throw err;
            });
            connectPromise
    ...
    async ensureFirstTopLevelPage(timeoutMs) {
        if (this.hasTopLevelPage())
            return;
        try {
            ...
            if (!(err instanceof sdkErrors_js_1.TimeoutError)) {
                throw err;
            }
    ...
    async waitForFirstTopLevelPage(timeoutMs) {
        ...
        throw new sdkErrors_js_1.TimeoutError(WAIT_FOR_FIRST_TOP_LEVEL_PAGE_OPERATION, timeoutMs);
    }
    ...
    async ensurePiercer(session) {
        ...
        if (this._piercerInstalled.has(id))
            return true;
        const installed = await (0, piercer_js_1.installV3PiercerIntoSession)(session);
        if (installed) {
    ...
    async addInitScript(script, arg) {
        const source = await (0, initScripts_js_1.normalizeInitScriptSource)(script, arg);
    ...
    get clipboard() {
        return (this._clipboard ??= new clipboard_js_1.ContextClipboard({
            ...
            resolvePage: async (page) => {
                ...
                if (!active)
                    throw new sdkErrors_js_1.PageNotFoundError("active page");
                return active;
            },
        }));
    }
    ...
    async getFullFrameTreeByMainFrameId(rootMainFrameId) {
        ...
        if (!owner)
            throw new sdkErrors_js_1.PageNotFoundError(`mainFrameId=${rootMainFrameId}`);
    ...
    async onAttachedToTarget(info, sessionId) {
        ...
        executionContextRegistry_js_1.executionContexts.attachSession(session);
        ...
        try {
            ...
            try {
                ...
                if (owner) {
                    ...
                    void executionContextRegistry_js_1.executionContexts
    ...
    async awaitActivePage(timeoutMs) {
        ...
        if (immediate)
            return immediate;
        throw new sdkErrors_js_1.PageNotFoundError("awaitActivePage: no page available");
    }
    /**
    ...
    async cookies(urls) {
        ...
        return (0, cookies_js_1.filterCookies)(mapped, urlList);
    }
    /**
    ...
    async addCookies(cookies) {
        const normalized = (0, cookies_js_1.normalizeCookieParams)(cookies);
        if (!normalized.length)
            return;
        const cdpCookies = normalized.map(cookies_js_1.toCdpCookieParam);
        try {
            ...
            throw new sdkErrors_js_1.CookieSetError(`Failed to set cookies [${names}] — ` +
    ...
    async clearCookies(options) {
        const hasFilter = options?.name !== undefined ||
        ...
        const toKeep = current.filter((c) => !(0, cookies_js_1.cookieMatchesFilter)(c, options));
        ...
        if (toKeep.length) {
            try {
                await this.conn.send("Storage.setCookies", {
                    cookies: toKeep.map(cookies_js_1.toCdpCookieParam),
                });
            }
            catch (err) {
                const detail = err instanceof Error ? err.message : String(err);
                const names = toKeep.map((c) => `"${c.name}"`).join(", ");
                throw new sdkErrors_js_1.CookieSetError(`clearCookies: cookies were cleared but failed to re-add the ${toKeep.length} ` +
