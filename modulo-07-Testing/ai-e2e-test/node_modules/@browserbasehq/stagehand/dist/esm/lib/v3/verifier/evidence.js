...
export function isImageEvidence(e) {
    return "bytes" in e && e.bytes instanceof Buffer;
}
export function isTextEvidence(e) {
 ...
 */
export async function loadAndReduceScreenshots(trajectory, opts = {}) {
    ...
    const stepIndexToCanonical = new Map();
    ...
    // ── Resize ─────────────────────────────────────────────────────────────
    const screenshots = await Promise.all(keptRaw.map(async (raw, canonicalIndex) => {
        const resized = sharp
 ...
 */
export async function collectCanonicalEvidence(trajectory, opts = {}) {
    ...
    const addTextEvidence = (stepIdx, source, text) => {
    ...
    for (let i = 0; i < trajectory.steps.length; i++) {
        ...
        for (const modality of step.agentEvidence?.modalities ?? []) {
            if (modality.type === "text") {
                ...
                addTextEvidence(i, "agent-json", safeStringifyEvidence(modality.content));
            }
        }
        ...
        addTextEvidence(i, "tool-output", safeStringifyEvidence(step.toolOutput?.result));
    }
    ...
    addTextEvidence(trajectory.steps.length, "probe-aria", trajectory.finalObservation?.ariaTree);
    ...
    const screenshotIdxToCombined = new Map();
    ...
    const remappedStepIndex = new Map();
    for (const [stepIdx, screenshotIdx] of loaded.stepIndexToCanonical) {
        ...
        if (combined !== undefined)
            remappedStepIndex.set(stepIdx, combined);
    }
    return {
        evidence,
        loaded: {
...
// ─── Internals ────────────────────────────────────────────────────────────
function safeStringifyEvidence(value) {
 ...
 */
async function resizePng(sharp, bytes, scaleFactor) {
 ...
 * means more similar.
 */
async function calculateMSE(sharp, img1, img2) {
 ...
 */
async function calculateSSIM(sharp, img1, img2) {
    const size = { width: 400, height: 300 };
