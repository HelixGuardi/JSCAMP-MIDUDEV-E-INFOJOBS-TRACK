...
export function getZFactory(schema) {
 ...
 */
export function stripModelProvider(modelId) {
...
export function getZodType(schema) {
 ...
 */
export function transformSchema(schema, currentPath) {
    ...
    if (isKind(schema, "pipe") && isZod4Schema(schema)) {
        ...
        if (newIn !== inSchema || newOut !== outSchema) {
            const result = z.pipe(newIn, newOut);
 ...
 */
export function injectUrls(obj, path, idToUrlMapping) {
    if (path.length === 0)
        return;
    const toId = (value) => {
 ...
 */
export const providerEnvVarMap = {
 ...
 */
export function loadApiKeyFromEnv(provider, logger) {
    if (!provider) {
        return undefined;
    }
...
export function hasModelProviderAuth(clientOptions) {
...
export function getInheritableModelOptions(clientOptions) {
    if (!clientOptions) {
        return undefined;
    }
...
export function trimTrailingTextNode(path) {
