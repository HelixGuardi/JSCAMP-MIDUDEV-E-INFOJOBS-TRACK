...
export class V3Context {
    ...
    get clipboard() {
        return (this._clipboard ??= new ContextClipboard({
            ...
            resolvePage: async (page) => {
                ...
                if (!active)
                    throw new PageNotFoundError("active page");
                return active;
            },
        }));
    }
    ...
    async onAttachedToTarget(info, sessionId) {
        ...
        try {
            ...
            if (isTopLevelPage(info)) {
                ...
                try {
                    page = await Page.create(this.conn, session, info.targetId, this.apiClient, this.localBrowserLaunchOptions, this.env === "BROWSERBASE");
                }
                catch (error) {
                    createError = error;
                }
    ...
    async addCookies(cookies) {
        ...
        try {
            ...
            throw new CookieSetError(`Failed to set cookies [${names}] — ` +
    ...
    async clearCookies(options) {
        ...
        if (toKeep.length) {
            try {
                ...
                throw new CookieSetError(`clearCookies: cookies were cleared but failed to re-add the ${toKeep.length} ` +
